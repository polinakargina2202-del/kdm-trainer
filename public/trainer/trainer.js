...
function initTopicPractice(root) {
  ...
  const render = () => {
    ...
    root.innerHTML = `<div class="kdm-widget-head"><div><span class="kdm-kicker">WISSENSCHECK</span><h2>Prüfe dich selbst</h2><p>Automatisch auswertbare Aufgaben zu dieser Lernseite. Die Aufgaben werden beim Neustart neu gemischt.</p></div><div class="kdm-score-pill">0 / ${chosen.length}</div></div><div class="kdm-questions"></div><div class="kdm-summary" hidden></div><button type="button" class="kdm-btn secondary kdm-new">Neue Aufgaben</button>`;
...
const CALCULATORS = {
  ...
  notice() {
    const b=calcShell('Kündigungsfrist','Datum und Betriebszugehörigkeit ändern; die Frist wird anhand der Tabelle auf der Lernseite bestimmt. Hinweis: Das Beispiel auf der Lernseite nennt bei 13 Jahren 6 Monate und widerspricht damit der dortigen Tabelle; dieser Rechner folgt der Tabelle.');const years=nInput('Betriebszugehörigkeit',13,1,0,40,'Jahre'); const date=document.createElement('label');date.className='kdm-field';date.innerHTML='<span>Kündigung erklärt am</span><input type="date" value="2025-12-11">';b.querySelector('.kdm-calc-inputs').append(years,date);const out=b.querySelector('.kdm-steps');function monthsFor(y){if(y<.5)return{weeks:2,any:true};if(y<2)return{weeks:4,mid:true};if(y<5)return{months:1};if(y<8)return{months:2};if(y<10)return{months:3};if(y<12)return{months:4};if(y<15)return{months:5};if(y<20)return{months:6};return{months:7};}function calcEnd(d,y){const rule=monthsFor(y);const dt=new Date(d+'T12:00:00');if(rule.any){dt.setDate(dt.getDate()+14);return{rule:'2 Wochen (jederzeit)',end:dt};}if(rule.mid){const base=new Date(dt);base.setDate(base.getDate()+28);const y2=base.getFullYear(),m=base.getMonth(),day=base.getDate(); if(day<=15)return{rule:'4 Wochen zum 15. oder Monatsende',end:new Date(y2,m,15)};return{rule:'4 Wochen zum 15. oder Monatsende',end:new Date(y2,m+1,0)};}const start=new Date(dt.getFullYear(),dt.getMonth()+1,1);const end=new Date(start.getFullYear(),start.getMonth()+rule.months+1,0);return{rule:`${rule.months} Monat${rule.months>1?'e':''} zum Monatsende`,end};}
    updateOnInputs(b,()=>{const Y=+years.querySelector('input').value,D=date.querySelector('input').value;if(!D)return;const r=calcEnd(D,Y);out.innerHTML=`<ol><li>Betriebszugehörigkeit: <strong>${fmt(Y)} Jahre</strong></li><li>Tabellenstufe: <strong>${r.rule}</strong></li><li>Ermitteltes Fristende: <strong>${r.end.toLocaleDateString('de-DE')}</strong></li></ol><div class="kdm-result">${r.end.toLocaleDateString('de-DE')}</div>`;});b.querySelector('.kdm-variant').remove();return b;
  }
};
...
function initExam(root) {
  ...
  function runSection(section){clearInterval(timerId); const cfg=EXAM_CONFIG[section], pool=shuffle(QUESTION_BANK.filter(q=>q.section===section)).slice(0,cfg.count), timed=root.querySelector('.kdm-timer-toggle input').checked; let seconds=cfg.minutes*60; stage.hidden=false; stage.innerHTML=`<div class="kdm-exam-bar"><div><span>Teil</span><strong>${SECTION_LABELS[section]}</strong></div><div class="kdm-timer">${timed?`${cfg.minutes}:00`:'ohne Zeitlimit'}</div></div><div class="kdm-exam-meta">${pool.length} Aufgaben · Ergebnis wird auf 100 Punkte normiert</div><div class="kdm-exam-questions"></div><button type="button" class="kdm-btn primary kdm-submit-exam">Teil abgeben</button>`; stage.scrollIntoView({behavior:'smooth',block:'start'}); const list=stage.querySelector('.kdm-exam-questions'); pool.forEach((q,i)=>{const wrap=document.createElement('div');wrap.className='kdm-exam-qwrap';wrap.innerHTML=`<div class="kdm-qnum">${i+1}</div>`;wrap.appendChild(renderQuestion(q,{exam:true}));list.appendChild(wrap);});
